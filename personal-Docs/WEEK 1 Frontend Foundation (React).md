...
// Response interceptor to handle global error messages or special statuses
apiClient.interceptors.response.use(
  (response) => {
    logger.api(response.config.method, response.config.url, response.status, response.duration || 0); // Assuming you might add a duration calculator
  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      // AuthContext will handle redirect or update user state
    }
